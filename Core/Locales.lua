---@class addonTableGatherOverview
local addonTable = select(2, ...)

addonTable.Locales = CopyTable(GATHEROVERVIEW_LOCALES.enUS)

for key, translation in pairs(GATHEROVERVIEW_LOCALES[GetLocale()]) do
  addonTable.Locales[key] = translation
end
for key, translation in pairs(addonTable.Locales) do
  _G["GATHEROVERVIEW_L_" .. key] = translation

  if key:match("^BINDING") then
    _G["BINDING_NAME_" .. key:match("BINDING_(.*)")] = translation
  end
end